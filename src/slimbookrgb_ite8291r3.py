...
print("Configfiledir: {}".format(config_file))
print()
...
class keyboard_grid(Gtk.Grid):
    ...
    def setup(self):
        ...
        print('Switch  loaded: '+str(switch.get_state()))

        switch.set_halign(Gtk.Align.END)
        switch.connect("state-set", self.kb_switch_light)
    ...
    # Grid attach KB --------------------------------------------------
        ...
        self.grid.attach(kb_switch, 0, 0, 1, 1)
        self.grid.attach(switch, 3, 0, 1, 1)
        self.grid.attach(label2, 0, 1, 1, 1)
        self.grid.attach(scale, 2, 1, 2, 1)

        self.grid.attach(Gtk.Separator(), 0, 2, 5, 1)

        self.grid.attach(colors_lbl, 0, 3, 5, 1)
        self.grid.attach(btn_box, 0, 4, 5, 1)

        self.grid.attach(Gtk.Separator(), 0, 5, 5, 1)
        self.grid.attach(label_effects, 0, 6, 5, 1)
        self.grid.attach(effects_combo, 0, 7, 5, 1)
        self.grid.attach(btn_folder, 5, 7, 1, 1)

        self.grid.attach(icon, 5, 0, 3, 7)
        self.show_all()
...
class SlimbookRGB1(Gtk.Window):
    ...
    def __init__(self):
        ...
        if os.path.exists(HOMEDIR+"/.config/autostart/slimbookrgbkeyboard-autostart.desktop"):
            ...
            os.system("cp {}/slimbookrgbkeyboard-autostart.desktop {}/.config/autostart/slimbookrgbkeyboard-autostart.desktop".format(CURRENT_PATH,HOMEDIR))
