...
USER = "/home/"+subprocess.getoutput("logname")
...
class SlimbookRGB1(Gtk.Window):
    ...
    def __init__(self):
        ...
        if not os.path.exists(USER+"/.config/autostart/slimbookrgbkeyboard-autostart.desktop"):
            print("Path exists")
        else:
            os.system("cp "+currpath+"/slimbookrgbkeyboard-autostart.desktop "+USER+"/.config/autostart/slimbookrgbkeyboard-autostart.desktop")
    ...
    # Try to load config last effect on combobox
        try:
            ...
            #If last value contains '/' --> It's a route to an anim file so we get just te anim
            if not config_effect.find("/")== -1:
    ...
    def kb_effect_changed(self, combo):
        ...
        if tree_iter is not None:
            ...
            #Chek if it's an effect or an animation
                ...
                call = subprocess.getstatusoutput('pkexec slimbookrgbkeyboard-applyconfig-pkexec')
                ...
                self.update_config_field("effect",command)
                os.system('pkexec slimbookrgbkeyboard-applyconfig-pkexec')
                print("\nAnim applied")
    ...
    def check_config(self):
        if os.path.isfile(USER+'/.config/slimbookrgbkeyboard/slimbookrgbkeyboard.conf'):
            print('File .conf already exists\n')           
        else:
            print ("File doesn't exist")

            if os.path.exists (USER+'/.config/slimbookrgbkeyboard'):
                print('Directory already exists')
                os.system('sudo -u '+USER_NAME+' touch '+USER+'/.config/slimbookrgbkeyboard/slimbookrgbkeyboard.conf')
                print('Creating file')

                with open( USER + '/.config/slimbookrgbkeyboard/slimbookrgbkeyboard.conf', 'w') as conf:
                    self.fichero_conf().write(conf)
                
                print('File created succesfully!\n')
                
                print(os.system("cat "+ USER + "/.config/slimbookrgbkeyboard/slimbookrgbkeyboard.conf"))

            else:
                print("Directory doesen't exist")
                os.system('mkdir '+USER+'/.config/slimbookrgbkeyboard')
                os.system('touch '+USER+'/.config/slimbookrgbkeyboard/slimbookrgbkeyboard.conf')
                print('Creating file')
    ...
    def update_config_field(self, variable, value):
        ...
        fichero = USER + '/.config/slimbookrgbkeyboard/slimbookrgbkeyboard.conf'
        config = configparser.ConfigParser()
        config.read(fichero)
